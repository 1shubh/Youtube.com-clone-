...
const playvideo = async () =>{
    ...
    container.append(iframe,name,chName,Desc)
...
const recommend = async () =>{
    try{
        ...
        console.log(data2)
        let res = await fetch(`https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=10&relatedToVideoId=${data2.videoId}&type=video&key=AIzaSyBGyYPfAirT_LY_1YCFUV_FyT2GxQ8ACQQ`)
...
const appendrecommend = async(val) =>{
    ...
    if(val===undefined){
       ...
       const recomContainer = document.getElementById("recommend")
        ...
        val.forEach(function({snippet,id}){
